...
#ifdef GPU
    if(gpu_index >= 0){
        cl_free(net->input_gpu);
		cl_free(net->truth_gpu);
...
std::thread train_network_in_thread(network *net, dataDark d, float *err)
{
    ...
    return std::thread(train_thread,(void*)ptr);
}
...
void pull_weights(layer l)
{
    if(l.type == CONVOLUTIONAL || l.type == DECONVOLUTIONAL){
        cl_pull_array(l.biases_gpu, l.bias_updates, l.n);
		cl_pull_array(l.weights_gpu, l.weight_updates, l.nweights);
        if(l.scales) cl_pull_array(l.scales_gpu, l.scale_updates, l.n);
    } else if(l.type == CONNECTED){
		cl_pull_array(l.biases_gpu, l.bias_updates, l.outputs);
		cl_pull_array(l.weights_gpu, l.weight_updates, l.outputs*l.inputs);
    }
}
...
void push_weights(layer l)
{
    if(l.type == CONVOLUTIONAL || l.type == DECONVOLUTIONAL){
        ...
        if(l.scales) cl_push_array(l.scales_gpu, l.scales, l.n);
    } else if(l.type == CONNECTED){
		cl_push_array(l.biases_gpu, l.biases, l.outputs);
		cl_push_array(l.weights_gpu, l.weights, l.outputs*l.inputs);
    }
}
...
{
    if (l.type == CONVOLUTIONAL || l.type == DECONVOLUTIONAL) {
		cl_push_array(l.biases_gpu, base.biases, l.n);
		cl_push_array(l.weights_gpu, base.weights, l.nweights);
        if (base.scales) cl_push_array(l.scales_gpu, base.scales, l.n);
    } else if (l.type == CONNECTED) {
		cl_push_array(l.biases_gpu, base.biases, l.outputs);
		cl_push_array(l.weights_gpu, base.weights, l.outputs*l.inputs);
    }
}
...
std::thread sync_layer_in_thread(network **nets, int n, int j)
{
    ...
    ptr->n = n;
    ptr->j = j;
    return std::thread(sync_layer_thread,(void*)ptr);
}
...
void sync_nets(network **nets, int n, int interval)
{
    int j;
    int layers = nets[0]->n;
	std::thread *threads = new std::thread[layers];
...
void pull_network_output(network *net)
{
    ...
    cl_pull_array(l.output_gpu, l.output, l.outputs*l.batch);
}
