...
std::chrono::steady_clock::time_point what_time_is_it_now()
{
...
double sec(std::chrono::steady_clock::time_point end, std::chrono::steady_clock::time_point start)
{
...
listDark *split_str(char *s, char delim)
{
    size_t i;
    size_t len = strlen(s);
    listDark *l = make_list();
    list_insert(l, s);
    for(i = 0; i < len; ++i){
        if(s[i] == delim){
            s[i] = '\0';
...
//The above 6 functions didn't use in other place.
/////////////////////////////////////
...
listDark *parse_csv_line(char *line)
{
    listDark *l = make_list();
    char *c, *p;
    int in = 0;
