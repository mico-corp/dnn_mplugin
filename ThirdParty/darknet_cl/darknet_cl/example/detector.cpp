...
void train_detector(char *datacfg, char *cfgfile, char *weightfile, int *gpus, int ngpus, int clear)
{
    ...
    std::thread load_thread = load_data(args);
...
void print_imagenet_detections(FILE *fp, int id, detection *dets, int total, int classes, int w, int h)
{
    ...
    for(i = 0; i < total; ++i){
        ...
        for(j = 0; j < classes; ++j){
            int category = j;
            if (dets[i].prob[category]) fprintf(fp, "%d %d %f %f %f %f %f\n", id, j+1, dets[i].prob[category],
                    xmin, ymin, xmax, ymax);
        }
    }
}
...
void validate_detector_flip(char *datacfg, char *cfgfile, char *weightfile, char *outfile)
{
    int j;
    listDark *options = read_data_cfg(datacfg);
    const char *valid_images = option_find_str(options, "valid", "data/train.list");
	const char *name_list = option_find_str(options, "names", "data/names.list");
    ...
    listDark *plist = get_paths(valid_images);
    char **paths = (char **)list_to_array(plist);
    ...
    if(0==strcmp(type, "coco")){
        if(!outfileName) outfileName = "coco_results";
        snprintf(buff, 1024, "%s/%s.json", prefix, outfileName);
        fp = fopen(buff, "w");
        fprintf(fp, "[\n");
        coco = 1;
    } else if(0==strcmp(type, "imagenet")){
        if(!outfileName) outfileName = "imagenet-detection";
        snprintf(buff, 1024, "%s/%s.txt", prefix, outfileName);
        fp = fopen(buff, "w");
        imagenet = 1;
        classes = 200;
    } else {
        if(!outfileName) outfileName = "comp4_det_test_";
        fps = (FILE **)calloc(classes, sizeof(FILE *));
        for(j = 0; j < classes; ++j){
            snprintf(buff, 1024, "%s/%s%s.txt", prefix, outfileName, names[j]);
            fps[j] = fopen(buff, "w");
        }
    }
	...
	std::chrono::steady_clock::time_point start = what_time_is_it_now();
...
void validate_detector_recall(char *cfgfile, char *weightfile)
{
    ...
    listDark *plist = get_paths("data/coco_val_5k.list");
    char **paths = (char **)list_to_array(plist);
...
void test_detector(const char *datacfg, char *cfgfile, char *weightfile, char *filename, float thresh, float hier_thresh, char *outfile, int fullscreen)
{
    listDark *options = read_data_cfg(datacfg);
    const char *name_list = option_find_str(options, "names", "data/names.list");
    char **names = get_labels(name_list);
