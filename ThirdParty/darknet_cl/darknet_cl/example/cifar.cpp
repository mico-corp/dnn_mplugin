...
void test_cifar_multi(char *filename, char *weightfile)
{
    ...
    for(i = 0; i < test.X.rows; ++i){
        ...
        int category = max_index(test.y.vals[i], 10);
        if(index == category) avg_acc += 1;
        free_image(im);
        printf("%4d: %.2f%%\n", i, 100.*avg_acc/(i+1));
    }
}
...
const char *labels[] = {"airplane","automobile","bird","cat","deer","dog","frog","horse","ship","truck"};
    ...
    for(i = 0; i < train.X.rows; ++i){
        ...
        char buff[256];
        sprintf(buff, "data/cifar/train/%d_%s",i,labels[category]);
    ...
    for(i = 0; i < test.X.rows; ++i){
        ...
        int category = max_index(test.y.vals[i], 10);
        char buff[256];
        sprintf(buff, "data/cifar/test/%d_%s",i,labels[category]);
        save_image_png(im, buff);
    }
}
